"""Integration tests for PerPixelRegressionTask and PerPixelRegressionHead."""
...
@pytest.fixture
def regression_dataset(tmp_path: pathlib.Path) -> Dataset:
    """Create a sample dataset with raster input, targets, and output layers.

    The dataset has:
    - An "image" layer with a single-band raster image (input)
    - A "targets" layer with regression target values
    - A "predictions" layer to store the output regression predictions
    """
...
@pytest.fixture
def regression_model_config() -> dict:
    """Model config using Swin-Tiny for per-pixel regression."""
    return {
        "model": {
                        ...
                        "encoder": [
                            {
                                ...
                                "init_args": {
                                    "input_channels": 1,
                                    "arch": "swin_v2_t",
                                    # Output multi-scale feature maps for UNet decoder.
                        ...
                        "decoder": [
                            {
                                ...
                                "init_args": {
                                    # in_channels matches Swin-Tiny output_layers=[1,3,5,7]
                                    "in_channels": [
                                        [4, 96],
                                        [8, 192],
                                        [16, 384],
...
def test_per_pixel_regression_prediction_writes_to_dataset(
    regression_dataset: Dataset,
    regression_model_config: dict,
    tmp_path: pathlib.Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test that per-pixel regression model config can be parsed and predictions are written.
    ...
    tmp_fname = tmp_path / "regression_model.yaml"
    with tmp_fname.open("w") as f:
        json.dump(regression_model_config, f)
...
def test_per_pixel_regression_training(
    regression_dataset: Dataset,
    regression_model_config: dict,
    tmp_path: pathlib.Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test that per-pixel regression model can be trained for one epoch.
    ...
    # Write config to file.
    tmp_fname = tmp_path / "regression_training.yaml"
    with tmp_fname.open("w") as f:
        json.dump(regression_model_config, f)
