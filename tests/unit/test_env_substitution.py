...
from rslearn.utils.env_substitution import (
    substitute_env_vars_in_string,
    substitute_env_vars_in_tree,
)
...
class TestEnvSubstitution:
    ...
    def test_substitute_env_vars_in_string_empty_string(self) -> None:
        ...
        result = substitute_env_vars_in_string("")
        assert result == ""
    ...
    def test_substitute_env_vars_in_tree_simple_dict(
        ...
        result = substitute_env_vars_in_tree(obj)
    ...
    def test_substitute_env_vars_in_tree_empty_objects(self) -> None:
        ...
        assert substitute_env_vars_in_tree(empty_dict) == {}
        assert substitute_env_vars_in_tree(empty_list) == []
        assert substitute_env_vars_in_tree(empty_tuple) == ()
    ...
    def test_substitute_env_vars_in_string_yaml_type_inference(
        ...
        assert substitute_env_vars_in_string("${INT_VAR}") == 42
        assert substitute_env_vars_in_string("${FLOAT_VAR}") == 3.14
        assert substitute_env_vars_in_string("${BOOL_TRUE_VAR}") is True
        assert substitute_env_vars_in_string("${BOOL_FALSE_VAR}") is False
        assert substitute_env_vars_in_string("${NULL_VAR}") is None
        assert substitute_env_vars_in_string("${STRING_VAR}") == "hello world"
        assert substitute_env_vars_in_string("${LIST_VAR}") == [
            1,
            2,
            3,
        ]
        assert substitute_env_vars_in_string("${DICT_VAR}") == {"key": "value"}

        # Test partial substitution (should remain strings)
        assert (
            substitute_env_vars_in_string("prefix_${INT_VAR}_suffix")
            == "prefix_42_suffix"
        )
        assert (
            substitute_env_vars_in_string("path/${STRING_VAR}/file")
            == "path/hello world/file"
        )

        # Test no substitution
        assert substitute_env_vars_in_string("no variables here") == "no variables here"
    ...
    def test_substitute_env_vars_in_string_yaml_error_handling(
        ...
        result = substitute_env_vars_in_string("${INVALID_YAML}")
    ...
    def test_substitute_env_vars_in_string_multiple_variables(
        ...
        for input_str, expected in test_cases:
            result = substitute_env_vars_in_string(input_str)
