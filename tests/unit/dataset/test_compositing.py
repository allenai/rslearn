"""Tests for rslearn.dataset.compositing."""
...
from rslearn.dataset.compositing import (
...
class TestLayerConfigCompositorInstantiation:
    """Test LayerConfig.instantiate_compositor for enum vs dict."""

    def test_enum_returns_builtin_compositor(self) -> None:
        """Enum compositing_method should return a built-in Compositor instance."""
        ...
        assert isinstance(compositor, FirstValidCompositor)

    def test_dict_instantiates_compositor(self) -> None:
        """A class_path dict should instantiate the compositor via jsonargparse."""
        layer_cfg = LayerConfig.model_validate(
            {
                "type": "raster",
                "band_sets": [{"bands": ["B1"], "dtype": "uint8"}],
                "compositing_method": {
                    "class_path": "rslearn.dataset.compositing.MeanCompositor",
                },
            }
        )
        compositor = layer_cfg.instantiate_compositor()
...
# ---------------------------------------------------------------------------
# Built-in compositor tests
# ---------------------------------------------------------------------------


class TestMeanCompositor:
    """Unit tests for MeanCompositor."""
...
class TestSpatialMosaicTemporalStackCompositor:
    """Tests for SpatialMosaicTemporalStackCompositor."""
...
class TestTemporalReducers:
    ...
    def test_temporal_reducers_clip_and_reduce(self, tmp_path: pathlib.Path) -> None:
        ...
        mean_result = TemporalMeanCompositor().build_composite(**common_kwargs)
        max_result = TemporalMaxCompositor().build_composite(**common_kwargs)
        min_result = TemporalMinCompositor().build_composite(**common_kwargs)
