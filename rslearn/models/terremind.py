...
TERRAMIND_MEANS = {
    "S2L1C": [
        2357.089, 2137.385, 2018.788, 2082.986, 2295.651, 2854.537, 
...
TERRAMIND_STDS = {
    "S2L1C": [
        1624.683, 1675.806, 1557.708, 1833.702, 1823.738, 1733.977, 
        1732.131, 1679.732, 1727.26, 1024.687, 442.165, 1331.411, 
        1160.419
    ],
    "S2L2A": [
...
class Terramind(torch.nn.Module):
    ...
    def __init__(
        ...
        if model_size == TerramindSize.BASE:
            self.model = BACKBONE_REGISTRY.build("terramind_v1_base", modalities=modalities, pretrained=True)
        elif model_size == TerramindSize.LARGE:
            self.model = BACKBONE_REGISTRY.build("terramind_v1_large", modalities=modalities, pretrained=True)
        else:
    ...
    def forward(self, inputs: list[dict[str, Any]]) -> list[torch.Tensor]:
        ...
        return [image_features.reshape(batch_size, self.height, self.width, -1).permute(0, 3, 1, 2)]
